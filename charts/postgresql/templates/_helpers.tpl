...
{{- end -}}
{{- if .Values.postgresql.password }}
databaseAdminPassword: {{ .Values.postgresql.adminPassword | quote }}
{{- else -}}
{{- $secret := lookup "v1" "Secret" .Release.Namespace  (include "postgresql.fullname" . ) -}}
{{- if $secret -}}
{{/*
   Reusing existing secret data
databaseAdminPassword: {{ $secret.data.databaseAdminPassword | quote }}
*/}}
databaseAdminPassword: {{ $secret.data.databaseAdminPassword | b64dec | quote }}
{{- else -}}
{{/*
    Generate new data
*/}}
databaseAdminPassword: "{{ randAlphaNum 20 }}"
{{- end -}}
{{- end -}}
